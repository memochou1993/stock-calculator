<script lang="ts">
  ...
  const title = '台股交易試算器｜手續費、損益金額、報酬率計算工具';
  const description = `台股交易試算器，提供股票、股票當日沖銷、指數股票型基金（ETF）交易試算。輸入交易類別、交易股數、買入價格、賣出價格、手續費折扣、最低手續費，可根據不同的成交價格，自動計算出股票成本、股票市值、買入手續費、賣出手續費、證券交易稅、交易成本、損益金額、報酬率。`;
